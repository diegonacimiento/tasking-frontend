...
export default function Header() {
  ...
  addEventListener("resize", () => {
    const newIsMobileScreen =
      window.innerWidth <= 712 || window.innerHeight <= 575;
    setIsMobileScreen(newIsMobileScreen);
  });

  return (
    <header>
      {token && isMobileScreen && <HeaderMobile logout={logout} />}
      ...
      {token && !isMobileScreen && <HeaderDesktop logout={logout} />}
